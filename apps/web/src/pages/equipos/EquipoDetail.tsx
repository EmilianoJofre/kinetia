...
export default function EquipoDetail() {
  ...
  if (!equipo) return <div className="text-center py-8 text-gray-400">Equipo no encontrado</div>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-sm text-gray-500">Temporada {equipo.temporada}</span>
            </div>
          </div>
        </div>
      ...
      {jugadores.length > 0 && (
        <div className="card">
          <h3 className="text-base font-semibold mb-4">Plantilla de jugadores</h3>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {jugadores.map((j) => (
                  ...
                  >
                    <td className="py-2 font-bold text-accent">{j.numero}</td>
                    <td className="py-2 font-medium">{j.nombre} {j.apellido}</td>
                    <td className="py-2">
                      <span className="px-2 py-0.5 bg-accent/10 text-accent text-xs rounded-full capitalize">{j.posicion}</span>
                    </td>
                    <td className="py-2">
                      <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${j.activo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
      ...
      {equipoPartidos.length > 0 && (
        <div className="card">
          <h3 className="text-base font-semibold mb-4">Partidos recientes</h3>
            ...
            {equipoPartidos.map((p) => (
              <div key={p.id} className="flex items-center justify-between p-3 bg-background rounded-lg">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-500">{new Date(p.fecha).toLocaleDateString('es')}</span>
                  <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${
                    p.estado === 'finalizado' ? 'bg-gray-100 text-gray-600' :
                    p.estado === 'en_curso' ? 'bg-green-100 text-green-700' :
